...
void main(void) {
    vec3 lightPosition01 = normalize(uLightPosition);
    vec3 worldNormal01 = normalize(vWorldNormal);

    vec3 pointLight = normalize(uLightPosition - vWorldPosition);

    vec3 directionToEye01 = normalize(uCameraPosition - vWorldPosition);
    vec3 reflection01 = 2.0 * dot(worldNormal01, pointLight) * worldNormal01 - pointLight;

    float lambert = max(dot(worldNormal01, pointLight), 0.0);
